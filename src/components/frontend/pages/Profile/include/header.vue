    ...
    <div class="col-12">
      <div class="card mb-4">
        <div
          ...
          <button
            v-if="!showSaveButton && !isPositionFixed"
            ...
            <div class="d-flex align-items-md-end align-items-sm-start align-items-center justify-content-md-between justify-content-start mx-4 flex-md-row flex-column gap-4">
              <div class="user-profile-info">
                ...
                <ul class="list-inline mb-0 d-flex align-items-center flex-wrap justify-content-sm-start justify-content-center gap-2">
                  <li class="list-inline-item d-flex gap-1 badge bg-primary bg-glow">
                    Developer
                  </li>
...
export default {
  data() {
    return {
      ...
      startY: 0,
      coverContainerHeight: 300, // Height of the cover container
  ...
  computed: {
    coverImageStyle() {
      return {
        position: "absolute",
        top: `${this.coverImagePosition.y}px`,
        left: "0px",
        width: "100%",
        height: "auto",
        cursor: this.isPositionFixed ? "default" : "move",
      };
    },
  },
  methods: {
    ...
    onDrag(event) {
      if (this.isDragging && !this.isPositionFixed) {
        ...
        // Set top limit and bottom limit based on image height
        const maxY = 0; // Top limit (image can't go above the top)
        const minY = this.coverContainerHeight - this.imageHeight; // Bottom limit (image can't go below the bottom)

        // Adjust position for drag limits
        if (this.imageHeight < this.coverContainerHeight) {
          // If image height is less than container height, restrict movement
          this.coverImagePosition.y = Math.min(maxY, newY); // Limit to the top
        } else {
          // If image height is greater or equal to the container height, allow full movement
          this.coverImagePosition.y = Math.max(minY, Math.min(maxY, newY));
        }
      }
    },
    ...
    onCoverPhotoSelected(event) {
      ...
      if (file) {
        ...
        img.onload = () => {
          this.imageHeight = img.height; // Store the height of the image
        };
      }
    },
    saveCoverPosition() {
      ...
      this.showCoverImage = true; // Ensure the cover image remains visible
    },
  },
};
</script>

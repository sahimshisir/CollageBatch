      ...
      <div class="d-flex col-12 col-lg-5 align-items-center p-4 p-sm-5">
        <div class="w-px-400 mx-auto">
          ...
          <p class="text-start mb-4">
            ...
            <span class="fw-medium d-block mt-2">{{ maskedEmail }}</span>
          </p>
...
export default {
  ...
  mounted() {
    ...
    this.emailshow  = localStorage.getItem("forgot_password_email" );
  ...
  computed: {
    maskedEmail() {
      if (!this.emailshow) return "";

      // Split the email into username and domain parts
      const [username, domain] = this.emailshow.split("@");

      // Show the first two and last two characters of the username, mask the middle characters
  ...
  methods: {
    ...
    async submitForm() {
      ...
      const forgot_password_email = localStorage.getItem(
        "forgot_password_email"
      );

      try {
        const response = await axios.post(
          `${this.globalVariables.apiUrl}/forgot_verify-otp`,
          {
    ...
    moveFocus(event, index) {
      ...
      if (currentInput.value.length === 1 && index < 5) {
        const nextInput = this.$refs[`otpInput${index + 1}`][0]; // Get the input element
        if (nextInput) {
          nextInput.focus(); // Only focus if the ref exists
        }
      } else if (currentInput.value.length === 0 && index > 0) {
        const previousInput = this.$refs[`otpInput${index - 1}`][0]; // Get the input element
        if (previousInput) {
          previousInput.focus(); // Only focus if the ref exists
        }
      }
    },
  },
